...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train/evaluate an XGBoost candidate selector for KGQA reranking."
    )
    ...
    cv = cross_validate_xgboost_ranker(
        data,
        n_folds=args.folds,
        seed=args.seed,
    ...
    model = train_final_xgboost_ranker(
        data,
        n_estimators=args.n_estimators,
        max_depth=args.max_depth,
        learning_rate=args.learning_rate,
    ...
    overall = cv["overall"]
    print("===== XGBOOST CV SUMMARY (Out-of-Fold) =====")
