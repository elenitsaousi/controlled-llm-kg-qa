...
try:
    ...
    # Prefer project root .env when running as a module
    load_dotenv(os.path.join(PROJECT_ROOT, ".env"))
    # Fallback to default search (cwd) if needed
    load_dotenv()
except Exception:
    pass
...
def _build_llm_client(choice: str, temperature: float):
    ...
    if choice == "infineon":
        if not os.environ.get("INFINEON_API_URL") or not os.environ.get(
            "INFINEON_API_KEY"
        ):
            raise RuntimeError(
                "Missing INFINEON_API_URL or INFINEON_API_KEY. "
...
def evaluate(
    ...
    if llm_client is None:
        raise RuntimeError(
            "No LLM client available. Set OPENAI_API_KEY, or set "
            "INFINEON_API_URL and INFINEON_API_KEY, or pass --llm openai/infineon."
        )
    ...
    summary["candidate_correct_rate"] = summary["correct_candidates"] / summary["total_candidates"]
    summary["candidate_invalid_rate"] = summary["invalid_candidates"] / summary["total_candidates"]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate Infineon questions by comparing candidate results to gold."
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument("--k", type=int, default=3, help="Candidates per question")
    parser.add_argument(
        "--schema",
        default="data/infineon/schema.json",
        help="Path to schema JSON (default: data/infineon/schema.json)",
    )
    parser.add_argument(
        "--llm",
        default="auto",
        choices=["auto", "openai", "infineon"],
        help="Which LLM backend to use for candidate generation",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.2,
        help="Sampling temperature for the LLM (set 0 for deterministic)",
    )
    parser.add_argument(
        "--progress",
        action="store_true",
        help="Print per-question progress while running",
    )
    parser.add_argument(
        "--query-timeout",
        type=float,
        default=None,
        help="Max seconds per SPARQL query (0/None disables timeout)",
    )
    parser.add_argument(
        "--out",
    ...
    print(f"Correct candidates: {summary['correct_candidates']}")
