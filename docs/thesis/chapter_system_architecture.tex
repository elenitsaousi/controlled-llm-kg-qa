...
In this thesis, entropy should be interpreted carefully. The entropy computed over candidate scores is not a direct measurement of how ambiguous the user's question is in a human-linguistic sense. It is a measurement of candidate-set uncertainty under the current scoring model. If several candidates receive similar probabilities, the model is uncertain about which interpretation should be selected. This often corresponds to user ambiguity, but it can also arise from near-duplicate candidates or from weak feature discrimination.
