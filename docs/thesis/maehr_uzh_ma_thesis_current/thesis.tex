...
The final system is evaluated on a mixed 1000-question benchmark covering True Demand KG analytics, ontology definition questions, and graph-grounded advisory questions. The system achieves 85.9\% audited answer-level accuracy while reducing LLM calls by 45.1\% compared with an all-LLM baseline. Deterministic graph-supported and ontology routes answer 44.8\% of the benchmark with 93.8\% accuracy. For the 552 fallback questions requiring candidate selection, guarded ML reranking improves Top-1 selection accuracy from 57.4\% to 68.3\%, while candidate coverage remains 91.3\%. The results show that LLM candidate generation is often successful, but reliable KGQA requires ambiguity-aware selection, deterministic routing, and graph-grounded validation.
...
Das entwickelte System kombiniert deterministische graphgestützte Weiterleitung, Definitionen aus der Digital-Reference-Ontologie, LLM-basierte SPARQL-Kandidatengenerierung, ML-basierte Query-Auswahl, Confidence-Routing, Ausführungsprüfungen und graphgestützte Antwortsynthese. In einer gemischten Evaluation mit 1000 Fragen erreicht das System 85.9\% geprüfte Antwortgenauigkeit und reduziert LLM-Aufrufe um 45.1\% gegenüber einer reinen LLM-Baseline. Die Ergebnisse zeigen, dass zuverlässige KGQA-Systeme nicht nur gute Kandidaten generieren müssen, sondern auch robuste Auswahl-, Validierungs- und Routing-Mechanismen benötigen.
...
\begin{itemize}
    ...
    \item It shows that the final system reaches 85.9\% audited answer-level accuracy while reducing LLM calls by 45.1\% compared with an all-LLM baseline.
    \item It demonstrates that guarded ML improves fallback candidate selection from 57.4\% to 68.3\% on the 552 questions that cannot be answered deterministically, while candidate coverage remains 91.3\%.
\end{itemize}
...
\section{Knowledge Graph Foundations Revisited}
\label{sec:kg-foundations-revisited}
...
thesis (Section~4.4) and the reported 45.1\% LLM-call reduction
...
(Section~5.2), which reached 87.9\% on the full selection benchmark and
91.3\% on the fallback subset (Table~6.1, Table~6.3), confirming that the
correct SPARQL candidate is usually present even when it is not selected
first.
...
pattern: schema-only selection achieves only 49.7\% Top-1 accuracy on the
full selection benchmark, \emph{lower} than raw LLM ordering at 53.9\%
(Table~6.1), and drops further to 42.2\% on the harder fallback subset,
compared to 57.4\% for raw LLM ordering and 68.3\% for guarded ML
(Table~6.3). Across both the text-to-SQL literature reviewed here and this
...
KG selection benchmark & 1000 KG questions & LLM-only generation and selection benchmark
(\path{true_demand_final_1000_gold.json}) \\
...
Full-system benchmark & 1000 mixed questions & Final end-to-end system benchmark
(\path{true_demand_full_system_1000.json}) \\
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{system_architecture_pipeline.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{direct_graph_supported_routing.png}
...
The final 1000-question audit supports this design choice. In that run, 448 questions were handled through deterministic graph-supported or ontology-supported routing, while 552 entered the LLM fallback route. This corresponds to 44.8\% deterministic coverage and 55.2\% LLM fallback coverage. Table~\ref{tab:architecture-audit-snapshot} summarizes the main architecture-level audit signals. These values are reported in more detail in the evaluation chapter, but they are useful here to justify the architectural separation between deterministic routing and candidate-based ranking.
...
Deterministic / graph-supported route & 448 questions (44.8\%) \\
LLM fallback route & 552 questions (55.2\%) \\
DR ontology definition route & 150 questions (15.0\%) \\
Advisory route & 50 questions (5.0\%) \\
Validated-retrieval selected source & 382 questions \\
Capability-inventory selected source & 262 questions \\
Infineon LLM selected source & 167 questions \\
Observed LLM calls & 549 calls \\
Estimated LLM-call reduction & 45.1\% \\
\bottomrule
\end{tabularx}
\end{table}
...
The candidate-generation prompt contains the user question, schema context, graph naming conventions, examples of valid SPARQL structures, and capability hints where available. The schema context restricts the model to known classes and predicates, while the capability hints encourage graph-supported analytical patterns. In some cases, a family-aware schema slice can be used to reduce prompt size by focusing on the most relevant graph area. If routing confidence is low, the system can use a broader schema context to avoid excluding the correct family too early.
...
The user interface is implemented as a Streamlit application. Its role is not only to collect a question and display an answer, but also to expose the system's decision process at the right level of detail. The interface supports natural-language question entry, KG-aware autocomplete, guided examples, clarification choices, technical SPARQL inspection, answer synthesis, evidence graph visualization, feedback, and developer diagnostics.

Figure~\ref{fig:ui-main-qa-flow} provides the main UI walkthrough. It combines the question-entry state and the answer state in one figure, so it explains the application without consuming too much thesis space. The first panel shows that the user can type a natural-language question and receive KG-aware suggestions. The second panel shows the synthesized answer, the route used by the system, and the evidence/explanation area.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/ui_main_qa_flow.png}
    \caption{Main QA workflow in the Streamlit application. Panel (a) shows the natural-language question input with KG-aware autocomplete suggestions. Panel (b) shows the graph-backed answer page, including the selected route and evidence-oriented output.}
    \label{fig:ui-main-qa-flow}
\end{figure}

Figure~\ref{fig:ui-answer-evidence-flow} can be used either in the main architecture chapter or in an appendix, depending on space. It is especially useful because it shows the evidence graph next to the answer. This helps explain how the application communicates that the response is grounded in graph execution rather than produced as a free-text LLM answer.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/ui_answer_evidence_flow.png}
    \caption{Answer and evidence view. The application presents the natural-language answer together with an answer evidence graph and feedback controls. This view supports transparency by connecting the answer to query-specific graph evidence.}
    \label{fig:ui-answer-evidence-flow}
\end{figure}
...
The system uses two different kinds of visual graph explanation. First, WebVOWL can be used to inspect the schema-level ontology. This is useful for understanding classes and relationships, but it does not show every instance-level survey record. Second, the Streamlit evidence graph is query-specific and shows the part of the graph related to the selected answer.

These two visualizations should not be confused. WebVOWL explains the conceptual structure of the ontology. The answer evidence graph explains why a particular query result is graph-backed. Figure~\ref{fig:webvowl-schema-view} shows the schema-level ontology view, while the Streamlit evidence graph remains query-specific.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/webvowl2.png}
    \caption{WebVOWL schema-level view of the True Demand ontology. This visualization is useful for exploring classes and relationships at the ontology level, while the Streamlit evidence graph is used for query-specific answer evidence.}
    \label{fig:webvowl-schema-view}
\end{figure}

\section{Developer and Deployment Controls}
\label{sec:developer-deployment-controls}

The application includes a developer mode because the architecture contains several configurable components: backend endpoint, model path, graph path, ranking policy, confidence thresholds, schema routing, candidate diagnostics, and graph-explorer settings. These controls are useful for development and evaluation, but they should not dominate the main thesis narrative. The main architecture chapter should briefly explain their purpose, while the detailed screenshots can be placed in an appendix.

Figure~\ref{fig:ui-developer-status} can be included in the architecture chapter if space allows. It shows the page navigation, system status, recommended developer configuration, and confidence-routing settings. The more detailed developer configuration screenshot can be moved to an appendix.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/ui_developer_status_combined.png}
    \caption{Developer and status controls in the Streamlit interface. These panels expose the runtime configuration, backend availability, model settings, confidence thresholds, and safety-guard options used during evaluation and debugging.}
    \label{fig:ui-developer-status}
\end{figure}

The developer controls support reproducibility because they make it possible to verify which backend, model, schema, and confidence configuration were active during a run. However, for normal business use these controls should be hidden or simplified so that the user interacts only with the question-answering and clarification workflow.
...
Fallback subset benchmark & 552 questions not answered deterministically in the final system & Measures raw/schema/ML selection on the exact questions that enter the fallback layer. \\
...
The first experiment evaluates the LLM candidate-generation and selection pipeline independently of deterministic routing. On the 1000-question KG selection benchmark, the raw LLM candidate order achieved 53.9\% Top-1 accuracy, while Any-Correct coverage reached 87.9\%. This means that in most cases the correct query was present somewhere in the generated candidate set, but the first selected candidate was often not the correct interpretation.
...
The gap between Top-1 accuracy and Any-Correct coverage is the most important observation in this experiment. A Top-1 value of 53.9\% means that the raw candidate order produced by the LLM is not reliable enough to be executed blindly. However, the Any-Correct value of 87.9\% means that the LLM frequently generated a correct query somewhere in the candidate set. The system is therefore not primarily failing because the LLM cannot express the graph query at all. Instead, it often fails because several plausible candidates are generated and the wrong one is ranked first. This justifies treating KGQA as a candidate-selection problem rather than as a single-query generation problem.
...
\midrule
Raw LLM order & 1000 & 53.9\% & 87.9\% & 12.1\% \\
Schema / semantic selection & 1000 & 49.7\% & 87.9\% & 12.1\% \\
Guarded ML selection & 1000 & 56.8\% & 87.9\% & 12.1\% \\
\bottomrule
\end{tabularx}
\end{table}
...
To avoid overfitting the final benchmark, a 200-question calibration split was separated from the 1000-question set, and the resulting model was evaluated on an untouched 800-question holdout split. On this holdout set, calibrated guarded ML improved Top-1 selection accuracy from 54.1\% raw LLM ordering to 59.8\%, while Any-Correct coverage remained 87.9\%. The improvement therefore comes from better selection, not from generating more correct candidates.

The holdout result is intentionally more conservative than the full-system result. It evaluates only whether the ranker selects the correct candidate from an existing candidate set. It does not include deterministic routing, ontology definitions, advisory templates, or answer-level equivalence. The improvement from 54.1\% to 59.8\% is therefore a direct measurement of the ML reranker's contribution to candidate selection. The fact that Any-Correct remains unchanged confirms that the model is not improving generation; it is exploiting features of the already generated candidates to select better among them.

\begin{table}[htbp]
\centering
\caption{Calibrated ML reranking on the 800-question holdout split.}
\label{tab:calibrated-ml-holdout}
\small
\begin{tabularx}{\textwidth}{
>{\raggedright\arraybackslash}X
>{\raggedleft\arraybackslash}p{1.8cm}
...
\toprule
\textbf{Mode} & \textbf{Questions} & \textbf{Top-1 accuracy} & \textbf{Any-Correct coverage} \\
\midrule
Raw LLM order & 800 & 54.1\% & 87.9\% \\
Previous guarded ML & 800 & 57.0\% & 87.9\% \\
Calibrated guarded ML & 800 & 59.8\% & 87.9\% \\
\bottomrule
\end{tabularx}
\end{table}

For the final system, it is also useful to evaluate only the questions that actually enter the fallback layer after deterministic routing. This subset contains 552 questions. On this fallback subset, raw LLM selection achieved 57.4\% Top-1 accuracy, schema-only selection achieved 42.2\%, and guarded ML achieved 68.3\%. Candidate coverage was 91.3\% for all three settings because the candidate set was the same; only the selection strategy changed.

\begin{table}[htbp]
\centering
\caption{Selection results on the 552-question LLM fallback subset.}
\label{tab:fallback552-selection}
\small
\begin{tabularx}{\textwidth}{
>{\raggedright\arraybackslash}X
...
\midrule
Raw LLM selection & 552 & 57.4\% & 91.3\% & 187 & 48 \\
Schema-only selection & 552 & 42.2\% & 91.3\% & 271 & 48 \\
Guarded ML selection & 552 & 68.3\% & 91.3\% & 127 & 48 \\
\bottomrule
\end{tabularx}
\end{table}

Guarded ML therefore improves the fallback selection layer by 10.9 percentage points over raw LLM ordering, corresponding to 60 additional correctly selected queries on the 552 fallback questions. This is the cleanest evidence that ML helps the selection component, while the Any-Correct value shows that candidate generation remains strong.

The fallback subset is especially important because it represents the questions that the final system could not answer deterministically. In other words, these are the questions for which candidate selection actually matters at runtime. The raw LLM order already solves 57.4\% of these cases, which shows that the first generated candidate is often useful. However, the 91.3\% Any-Correct coverage shows that there is much more information available in the candidate set than the first candidate alone. Guarded ML uses this information and raises Top-1 accuracy to 68.3\%. The remaining gap between 68.3\% Top-1 and 91.3\% Any-Correct represents the unresolved selection headroom: in many cases the correct query is present, but the system still does not select it.

The low schema-only result on the same subset further supports the argument that ambiguity is semantic, not only structural. These fallback questions are precisely the cases where deterministic capability matching was not sufficient. They often involve competing domains, similar metrics, missing scope, or subtle differences between a grouped breakdown and a top-value query. Schema-only selection does not capture these distinctions well, while guarded ML can use additional features such as query shape, aggregation, selected variables, candidate source, and score margins.
...
The final full-system audit contains 141 incorrect answers out of 1000 questions. Of these, 113 occur in the LLM fallback route. To understand whether these failures are avoidable, the LLM fallback errors were classified according to the difficulty of writing the correct SPARQL query manually.

\begin{table}[htbp]
\centering
\caption{Human SPARQL difficulty of incorrect LLM fallback answers.}
...
\toprule
\textbf{Difficulty} & \textbf{Errors} & \textbf{Share of LLM errors} & \textbf{Share of all questions} \\
\midrule
Easy & 53 & 46.9\% & 5.3\% \\
Medium & 37 & 32.7\% & 3.7\% \\
Hard & 23 & 20.4\% & 2.3\% \\
...
The easy failures mostly occur in shortage and vehicle-sales questions. These are cases where a human with schema knowledge could likely write the correct query by applying the appropriate shortage flag, actual/forecast filter, or units-sold aggregation. Medium failures include autonomous-driving and regional/future-demand groupings where the intent is understandable, but the correct graph path requires schema inspection. Hard failures include BL1/BL2 baseline modeling, future-demand combinations across region and quarter, and advisory questions that require both query execution and interpretation.

This analysis shows that the remaining failures are not all equally difficult. A substantial share of the LLM errors are still avoidable engineering failures, especially around answer-shape enforcement and missing filters. However, a smaller subset reflects genuinely difficult graph-composition problems where even a human would need to inspect the ontology, test candidate paths, and validate non-empty results.

The distribution in Table~\ref{tab:human-sparql-difficulty} is useful for interpreting the 113 LLM fallback errors. The 53 easy cases indicate remaining implementation weaknesses that should be addressed through stronger templates, filter checks, and answer-shape validation. These include cases where the question clearly asks for a shortage flag, actual versus forecast sales, or a simple count. A knowledgeable human would likely write the correct SPARQL query without extensive graph exploration.

The 37 medium cases are different. Here, the natural-language intent is usually understandable, but the correct graph path is not immediately obvious. Examples include autonomous-driving questions that combine vehicle type, SAE level, year, and survey provenance, or demand questions that require the correct distinction between total demand, percentage change, and survey scope. A human could answer these questions, but would probably inspect the schema and test one or two candidate queries.

The 23 hard cases represent the most important limitation of the current graph interface. These questions require non-obvious joins, derived interpretation, or paths that are not aligned with the way a user would naturally phrase the question. For such cases, it is reasonable that the LLM struggles. The error is not simply a language-understanding failure; it reflects a mismatch between natural business phrasing and the actual graph modeling structure.
...
In the final benchmark, 448 questions were routed deterministically, while 552 entered the LLM fallback layer. This split shows that the router reduced unnecessary LLM usage while still preserving a flexible candidate-generation route for harder questions.
...
The final 1000-question system benchmark evaluates the complete user-facing pipeline. The benchmark contains 800 True Demand KG questions, 150 DR ontology definition questions, and 50 advisory questions. The system answered 448 questions through deterministic graph-supported routing and 552 through the LLM fallback route.
...
The deterministic route is more accurate because it is narrower. It only fires when the capability resolver has enough evidence to identify a supported metric, dimension, aggregation, and graph path. This explains the 93.8\% deterministic accuracy. The route is not perfect because some deterministic patterns can still be too broad, can miss a subtle scope requirement, or can return a grouped answer when the user expected a ranked one. However, it is much more stable than the fallback route because it avoids open-ended candidate generation.

The LLM fallback route is lower at 79.5\% because it handles the harder part of the benchmark. These questions are precisely the ones that deterministic routing could not safely resolve. They contain more ambiguity, more schema-dependent joins, and more competing interpretations. The fallback accuracy should therefore not be interpreted as a weakness of the system alone; it reflects the higher difficulty of the subset routed to the LLM.
...
\midrule
True Demand KG analytics & 800 & 661 & 82.6\% \\
...
Observed LLM calls & 549 \\
All-LLM baseline calls & 1000 \\
Estimated cost at EUR 0.20 per call & EUR 109.80 \\
All-LLM baseline cost & EUR 200.00 \\
Estimated cost reduction & 45.1\% \\
\bottomrule
\end{tabularx}
\end{table}

The final benchmark also supports a conservative time-saving estimate. Manually formulating and validating SPARQL queries for 1000 mixed questions would require substantial expert time, especially for schema-dependent analytical questions. Under conservative assumptions, the manual effort would be on the order of dozens of analyst hours, while the KGQA system returns most answers within seconds and reduces the number of cases requiring expert inspection. This estimate is not presented as an exact measurement, but it illustrates the practical value of the system for non-technical users.

\begin{table}[htbp]
\centering
\caption{Transparent assumptions for manual SPARQL effort estimation on incorrect LLM fallback cases.}
...
The assumptions in Table~\ref{tab:manual-sparql-effort} are intentionally conservative. They apply only to the 113 incorrect LLM fallback cases, not to the full benchmark, and they estimate the time a knowledgeable analyst would need to formulate and validate a correct SPARQL query. Under these assumptions, the remaining fallback errors alone correspond to about 25.2 hours of manual query-engineering effort. The estimate is not a replacement for a user study, but it anchors the time-saving claim in a transparent calculation.

The cost result follows directly from the routing architecture. In an all-LLM baseline, every question would require an LLM call, resulting in 1000 calls for the benchmark. The final system made 549 LLM calls because deterministic KG routes, DR ontology definitions, and advisory templates answered a substantial portion of the questions without generation. At an assumed cost of EUR 0.20 per call, this reduces the estimated cost from EUR 200.00 to EUR 109.80. The 45.1\% reduction is therefore not an abstract optimization; it is a direct consequence of moving answerable questions out of the LLM path.

The time-saving interpretation is similar. A human analyst writing SPARQL manually would need to understand the ontology, identify the correct graph path, test the query, inspect empty results, and summarize the output. The system automates this process for most questions and leaves only the difficult or incorrect cases for review. The exact time saved depends on the analyst's expertise, but the benchmark shows that the system can answer 859 out of 1000 mixed questions correctly and can do so with substantially fewer LLM calls than an all-LLM approach.
...
The strongest result is the combination of deterministic routing and LLM fallback. Deterministic routes answered 44.8\% of the final benchmark with 93.8\% accuracy. This shows that many recurring business questions do not require LLM generation at all if the graph capabilities are known and validated. At the same time, the LLM fallback layer remains useful for questions outside the deterministic capability inventory.

The second important result is that candidate generation is substantially stronger than candidate selection. On the 552 fallback questions, Any-Correct coverage reached 91.3\%, meaning that the correct query was usually present in the candidate set. Guarded ML improved Top-1 fallback selection from 57.4\% to 68.3\%. This supports the central claim that ambiguity-aware selection is a meaningful component in LLM-based KGQA.
...
Schema-only selection did not perform well on the fallback subset. It reduced Top-1 accuracy from 57.4\% for raw LLM ordering to 42.2\%. This indicates that schema validity can be misleading when several structurally valid candidates correspond to different business interpretations.
...
Another limitation is that the answer-level audit and the selection benchmark measure different things. The full system reached 85.9\% answer-level correctness, while guarded ML reached 68.3\% Top-1 selection on the fallback subset. These numbers should not be compared directly. The former evaluates the final user-facing answer; the latter evaluates whether the selected candidate matches the intended query candidate.
...
\begin{itemize}
    ...
    \item \textbf{Cost control}: deterministic and ontology routes reduce LLM calls by 45.1\% compared with an all-LLM baseline.
...
For reporting purposes, a conservative claim is that the system reduces manual KG-querying effort substantially while reaching 85.9\% audited answer accuracy on a mixed 1000-question benchmark. The exact time saving depends on the expertise of the human analyst and the complexity of the question, but the system clearly reduces the need for manual SPARQL formulation in a large share of cases.
...
The ML reranker improves selection, but it does not solve all semantic ambiguity. More expressive semantic features, embeddings, or preference-learning approaches may improve the ranking of close candidates. However, high-ambiguity cases will still require clarification or abstention when the user intent cannot be inferred from the question alone.
...
The final evaluation shows that the hybrid architecture improves both reliability and efficiency. On a mixed 1000-question benchmark, the system achieved 85.9\% audited answer accuracy. It answered 44.8\% of questions through deterministic graph-supported routing and reduced LLM calls by 45.1\% compared with an all-LLM baseline. On the 552 fallback questions that required candidate selection, guarded ML improved Top-1 selection accuracy from 57.4\% to 68.3\%, while candidate coverage remained 91.3\%.
...
Second, the ML reranker can be improved with richer semantic features. The current model uses structured query and schema-derived features. Embedding-based similarity between the natural-language question and candidate query descriptions could help distinguish candidates that are structurally similar but semantically different. The grouped candidate-query dataset could also support preference-learning approaches, such as DPO-style fine-tuning of the query generator. This would not eliminate the need for ambiguity estimation, but it may reduce the number of semantically incorrect candidates generated in the first place.
