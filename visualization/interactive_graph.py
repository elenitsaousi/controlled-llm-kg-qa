...
def _build_svg_graph_html(
    ...
    *,
    ...
    return (
        ...
        "#svgNodePanel{width:300px;flex:0 0 300px;background:#172331;border-left:1px solid #243344;color:#dbe7ee;overflow:auto;font-size:13px;line-height:1.45;}"
        ...
        "@media(max-width:640px){.svgShell{flex-direction:column;}#graphWrap{min-width:0;}#svgNodePanel{width:100%;flex:auto;max-height:240px;}}"
        "</style>"
        ...
        "function updateLabelVisibility(){document.querySelectorAll('.edgeLabel').forEach(e=>e.style.display=scale>2.15?'block':'none');document.querySelectorAll('.nodeLabel').forEach(e=>{const selected=e.parentElement&&e.parentElement.dataset.nodeId===selectedNodeId;e.style.display=(selected||scale>1.12)?'block':'none';e.setAttribute('font-size',scale>2.0?'15':'12');});document.querySelectorAll('.edgePath').forEach(e=>{e.setAttribute('stroke-opacity',scale>1.15?'0.86':'0.70');e.setAttribute('stroke-width',scale>1.15?'1.6':'1.4');});document.querySelectorAll('.nodeCircle').forEach(c=>{c.setAttribute('r',scale>2.0?'18':'21');});}"
        "function applyGraph(){viewport.setAttribute('transform',`matrix(${scale} 0 0 ${scale} ${tx} ${ty})`);updateLabelVisibility();}"
        "function zoomGraph(f){scale=Math.max(0.25,Math.min(5,scale*f));applyGraph();}"
        "function fitGraph(){const box=viewport.getBBox();const vb=svg.viewBox.baseVal;const pad=90;const sx=(vb.width-(pad*2))/Math.max(1,box.width);const sy=(vb.height-(pad*2))/Math.max(1,box.height);scale=Math.max(0.45,Math.min(1.35,Math.min(sx,sy)));tx=(vb.width-(box.width*scale))/2-(box.x*scale);ty=(vb.height-(box.height*scale))/2-(box.y*scale);applyGraph();}"
        ...
        "window.addEventListener('mousemove',(e)=>{if(!dragging)return;const vb=svg.viewBox.baseVal;const ux=vb.width/Math.max(1,svg.clientWidth);const uy=vb.height/Math.max(1,svg.clientHeight);tx+=(e.clientX-sx)*ux;ty+=(e.clientY-sy)*uy;sx=e.clientX;sy=e.clientY;applyGraph();});"
